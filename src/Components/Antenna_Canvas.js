...
var ctx1,ctx2,ctx3;
export const Antenna_Canvas=({setting})=> {
  const [canvas_width,setwidth]=useState(0);
  const [canvas_height,setheight]=useState(0);
  const runonce=useRef(false);

  useEffect(()=>{
    if(setting!==undefined && Object.keys(setting).length==0){
      return;
    }
  ...
  useEffect(()=>{
   ...
   function draw_canvas(){
    var lambda_len=setting.lambda_field;
    var d=setting.d;
    var deg=setting.deg;
    ...
    line_triangle(space,tip_space,space,canvas_height-tip_space,10,lambda_len+" λ");
    line_triangle(tip_space,canvas_height-space,canvas_width-tip_space,canvas_height-space,12,lambda_len+" λ");
    
    const numCircles = 8;
    const radiusIncrement = 1/lambda_len*canvas_width;

    for (let i = 0; i < numCircles; i++) {
        const radius = i * radiusIncrement;
        ctx1.beginPath();
        ctx1.arc(canvas_width/2-len/2, canvas_height/2, radius, 0, 2 * Math.PI);
        ctx1.strokeStyle = "rgba(255,0,0,0.4)";
        ctx1.lineWidth = 1;
        ctx1.stroke();
        ctx1.closePath();
    }
    ...
    for (let i = 0; i < numCircles; i++) {
        const radius = i * radiusIncrement+radiusIncrement*deg/360;
        ctx1.beginPath();
        ctx1.arc(canvas_width/2+len/2, canvas_height/2, radius, 0, 2 * Math.PI);
        ctx1.strokeStyle = "rgba(0,0,255,0.4)";
        ctx1.lineWidth = 1;
        ctx1.stroke();
        ctx1.closePath();
    }
    ...
    function line_triangle(x1,y1,x2,y2,s,str_sentence){
        ...
        ctx1.strokeStyle="rgba(100,100,100,1.0)";
        ctx1.lineWidth=1;
        ctx1.beginPath();
        ctx1.moveTo(x1,y1);
        ctx1.lineTo(x2,y2);
        ...
        function triangle(x,y,s,rot){
          ...
          ctx1.lineTo(x+s*cos_x,y+s*sin_y);
          ctx1.closePath();
          ctx1.strokeStyle = "rgba(100,100,100,1.0)"; //枠線の色
          ctx1.stroke();
          ctx1.fillStyle="rgba(100,100,100,1.0)";//塗りつぶしの色
          ctx1.fill();
        }
